...
death_causes_acceptable <- death_causes_final[causa_inaceptable==F][,code:=icd_10_code][,
                                                                                        -c("causa_inaceptable","icd_10_code")]

barra_evol <- seq(1000,100000,by=1000)
system.time(
for(i in 1:nrow(mortalidad)){
        ...
        if(i==1){
           ...
           if(any(mortalidad[i,c("code","causa1","causa2","causa3","causa4","causa5",
                         "causa6")]%in%death_causes_acceptable$icd_10_code)){
                   prelim <- merge(mortalidad[i,],
                                   death_causes_acceptable[icd_10_code==mortalidad[i,code]][,-c("icd_10_code")],all=T, by="code")
           }else{
              ...
              if(any(mortalidad[i,c("code","causa1","causa2","causa3","causa4","causa5",
                   ...
                   prelim <- rbind(prelim,
                        ...
                        prelim <- rbind(prelim,mortalidad[i,][,`:=`(
                           aids_related="",
                           major_category="",
                           specific_category=""
                   )])
